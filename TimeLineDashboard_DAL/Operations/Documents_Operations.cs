...
{
    internal class Documents_Operations
    {
        #region Singleton
        private static readonly Documents_Operations instance = new Documents_Operations();
        // Explicit static constructor to tell C# compiler  
        // not to mark type as beforefieldinit  
        static Documents_Operations()
        {
        }
        private Documents_Operations()
        {
        }
        public static Documents_Operations Instance
        {
            get
            {
                return instance;
        ...
        internal List<Documents> Get_Search( string p_Search_Criteria , int p_User_Id )
        {
            List<Documents> Documents_To_Return = new List<Documents>();
            ...
            var dataSet = SQLHelper.SelectUsingStoredProcedure_WithDefaultAppConfigConnectionString("p_TLBoard_Get_Documents_Search", 
                new List<SqlParameter>() { spSearch_Criteria , spUser_Id });

            if (dataSet != null && dataSet.Tables[0].Rows.Count > 0)
            {
                Documents_To_Return = new List<Documents>(dataSet.Tables[0].Rows.Count);

                for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
                {
                    Documents_To_Return.Add(Create_Document_Details_From_Data_Row(dataSet.Tables[0].Rows[i]));
                }
            }
        ...
        internal Documents Get_Supplier_Details_By_Supplier_Id(int p_Supplier_Id, int p_User_Id_Supplier_Owner )
        {
            Documents Supplier_To_Return = new Documents();
            ...
            if (dataSet != null && dataSet.Tables[0].Rows.Count > 0)
            {
                Supplier_To_Return = Create_Document_Details_From_Data_Row(dataSet.Tables[0].Rows[0]); 
            }

            return Supplier_To_Return;
        }

        internal Documents Insert_New_Supplier_Administrative_Registration_Process(
        ...
        {
            Documents new_Registered_Supplier_To_Return = null;
        ...
        private Documents Create_Document_Details_From_Data_Row(DataRow dbRowDetailsForUserInitialization)
        {
            Documents Document_To_Return = new Documents();
